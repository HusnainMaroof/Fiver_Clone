...
const ThirdContent = ({ handleBack }) => {
  ...
  useEffect(() => {
    ...
    // If all rules are false (i.e., no errors), input is valid
    const isValid = Object.values(rules).every((val) => !val);
    setIsUserNameRulesValid(isValid);
    console.log(isValid, rules, isUserNameRulesValid);
  }, [userName]);
        ...
        <div>
          ...
          <div className="block py-1.5 w-full border-2 ps-1.5 rounded-md text-md font-semibold border-gray-300 focus:border-2 focus:border-gray-300 outline-0 placeholder:font-normal hover:border-black pr-1.5">
            <input
              ...
              className="ps-1 outline-0 border-0 w-full"
            />
          </div>
          ...
          {userName === "" || isUserNameRulesValid ? (
            ...
            <p className="text-red-600 py-1.5 text-sm font-semibold">
              {userNameRules.tooShort ? (
                ...
                <span>
                  Your username must begin with a letter and can include numbers
                  and underscores.
                </span>
